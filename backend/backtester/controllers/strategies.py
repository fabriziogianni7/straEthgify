from backtester.utils import get_token_dataset
import numpy as np
import json
...
def backtest_single_asset(asset, starting_balance, start_date, period):
  ...
  df_prices['benchmark_dd'] = df_prices['benchmark'] - df_prices['benchmark_peak']
  ...
  df_prices['strategy'] = starting_balance * df_prices['strategy_return'].cumprod()
  df_prices['strategy_peak'] = df_prices['strategy'].cummax()
  df_prices['strategy_dd'] = df_prices['strategy'] - df_prices['strategy_peak']

  benckmark_return = round(
    (df_prices['benchmark'][-1] / df_prices['benchmark'][1]) * 100,
    2)

  benckmark_max_drawdown = (df_prices['benchmark_dd'] / df_prices['benchmark_peak']).min() * 100

  strategy_return = round(
      (df_prices['strategy'][-1] / df_prices['strategy'][1]) * 100,
      2)

  strategy_max_drawdown = (df_prices['strategy_dd'] / df_prices['strategy_peak']).min() * 100

  df_prices.reset_index(inplace=True)
  df_prices['date'] = df_prices['date'].apply(lambda x: x.strftime('%Y-%m-%d'))
